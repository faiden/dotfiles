...
cal -m | sed -re "s/^(.*[A-Za-z][A-Za-z]*.*)$/^fg($HIGHLIGHT)^bg($BACKGROUND)\1/;s/(^|[ ])($TODAY)($|[ ])/\1^bg($HLTODAY)^fg($BACKGROUND)\2^fg($FOREGROUND)^bg($BACKGROUND)\3/" |dzen2 -p -bg "$BACKGROUND" -fg $FOREGROUND -l "$LINES" -w "$WIDTH" -x "$XPOS" -y "$YPOS" -sa c -e 'onstart=uncollapse;button1=exit;button3=exit' -title-name 'dzen-popup-cal' -fn 'lucy tewi'
